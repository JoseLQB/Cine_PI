      ...
      <div class="p-4 col-lg-8">
        <form action="compraConfirm.php" method="POST">
          <h5 class=""><b>Comprando entradas para:
              ...
              ?></b>
            <hr>Proyeccion:<br>
            <?php $objProyeccion = Proyecciones::getProyeccionesById($_POST["idPr"]);
            ...
            <?php foreach ($objProyeccion as $k) {
              $sala = $k->idSala;
              echo  $sala;
            }

            ?><br><br>Tarifa que se aplica:<br>
            <?php
            foreach ($objProyeccion as $k) {
              $tar = $k->codTarifa;
            }
            ...
            ?><br><br>Selecciona la cantidad que vas a comprar
            <?php
            $cantidad = 0;
            ?>
            <input type="number" name="cantidad" min="0" max="<?php echo $total ?>" id="" required>
            <br>
            <hr><button type="submit" class="btn btn-success" name="comprar" data-toggle="modal" data-target="#myModal"><i class="fa fa-cart-plus"></i>&nbsp;Comprar</button>
          </h5>
          <?php
